...
void main() {
  ...
  group('CommissionResolver.capEffectiveRate — plancher (minimum) et plafond (maximum effectif)', () {
    test('la commission minimum en \$ est appliquée quand le taux brut ne suffit pas', () {
      ...
      final effectiveRate = CommissionResolver.capEffectiveRate(
        ...
        maximumEffectiveCommissionRate: 1.0, // pas de plafond actif dans ce cas
      );
    ...
    test('le taux effectif ne dépasse jamais le maximum effectif configuré', () {
    ...
    test('quand le taux brut dépasse déjà le plancher, le taux nominal est conservé (sous le plafond)', () {
    ...
    test('missionBaseValue <= 0 retourne le taux résolu sans division par zéro', () {
      final effectiveRate = CommissionResolver.capEffectiveRate(
        resolvedRate: 0.12,
        missionBaseValue: 0,
        minimumPlatformCommission: 5,
  ...
  group('CommissionResolver.resolve — Founding Driver', () {
    ...
    test('applique le taux PROMOTIONNEL quand la qualification est dans la période promo', () {
    ...
    test('applique le taux PRÉFÉRENTIEL une fois la période promo terminée (qualification maintenue)', () {
      final qualification = FoundingDriverQualification(
        driverId: 'driver_001',
        programId: program.programId,
        status: FoundingDriverStatus.qualified,
    ...
    test('un Founding Driver RÉVOQUÉ retombe sur le taux standard (pas de priorité founding)', () {
      final qualification = FoundingDriverQualification(
        driverId: 'driver_001',
        programId: program.programId,
  ...
  group('CommissionResolver.resolve — promotion chauffeur expirée', () {
    test('une driver_promotion EXPIRÉE (endsAt dans le passé) ne doit JAMAIS être appliquée', () {
    ...
    test('une driver_promotion is_active=false ne doit JAMAIS être appliquée, même dans sa fenêtre temporelle', () {
    ...
    test('une driver_promotion VALIDE (active et dans sa fenêtre) est bien appliquée', () {
