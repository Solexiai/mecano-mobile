...
PricingConfig _config({double taxRate = 0.0, double serviceFeeRate = 0.0}) {
  return PricingConfig(
    ...
    surcharges: const [],
    customerServiceFee: CustomerServiceFeeConfig(
...
void main() {
  group(
    'CustomerPricingEngine.calculateQuote — calcul de base (sans remise)',
    () {
      ...
      test(
        'applique le minimumCharge quand le tarif brut calculé est trop bas',
        () {
          final config = _config();
          final result = CustomerPricingEngine.calculateQuote(
            config: config,
      ...
      test(
        'retourne un résultat neutre (unconfigured) si la config est inactive',
        () {
          final config = _config();
  ...
  group(
    'CustomerPricingEngine.calculateQuote — promotion client (customerDiscountAmount)',
    () {
      test(
        'une remise valide réduit le subtotal, et les frais de service + taxes sont calculés APRÈS remise',
        () {
          ...
          expect(
            withDiscount.customerTotal,
            lessThan(noDiscount.customerTotal),
          );
        },
      );

      test(
        'une remise supérieure au subtotal brut est plafonnée exactement au subtotal (jamais négatif)',
        () {
          final config = _config();
          final result = CustomerPricingEngine.calculateQuote(
            config: config,
            input: const CustomerPricingInput(
              ...
              customerDiscountAmount:
                  999999, // largement supérieure au subtotal
            ),
          );
      ...
      test(
        'une remise négative ou nulle est ignorée (aucun effet, pas de crash)',
        () {
          final config = _config();
          final result = CustomerPricingEngine.calculateQuote(
            config: config,
