...
void main() {
  ...
  group(
    'PaymentInfo.fromJson — statuts Phase 6 complets (aucun repli silencieux)',
    () {
      test(
        'chaque valeur PaymentStatuses serveur est reconnue individuellement',
        () {
          ...
          for (final entry in serverValues.entries) {
            ...
            expect(
              payment.status,
              entry.value,
              reason: 'status serveur "${entry.key}"',
            );
          }
        },
      );
      ...
      test(
        'un statut refunded est bien détecté par isRefunded (partiel ET complet)',
        () {
          final partial = PaymentInfo.fromJson(
            _fullPaymentJson()..['status'] = 'partially_refunded',
          );
          final full = PaymentInfo.fromJson(
            _fullPaymentJson()..['status'] = 'refunded',
          );
  ...
  group(
    'PaymentInfo.fromJson — rétro-compatibilité (ancienne mission, champs Phase 6 absents)',
    () {
      test(
        "un document minimal (seulement les champs requis) ne plante pas",
        () {
  ...
  group('PaymentInfo.copyWithLastRefundAt', () {
    test(
      'attache une date de dernier refund sans modifier les autres champs',
      () {
        final original = PaymentInfo.fromJson(_fullPaymentJson());
