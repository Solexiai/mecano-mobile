  ...
  Widget build(BuildContext context) {
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          Text(t('requests_title'), style: const TextStyle(fontSize: 22, fontWeight: FontWeight.w800)),
          const SizedBox(height: 16),
          Wrap(
            spacing: 10,
            children: [
              ChoiceChip(label: Text(t('requests_filter_all')), selected: _filter == 0, onSelected: (_) => setState(() => _filter = 0)),
              ChoiceChip(label: Text(t('requests_filter_delivery')), selected: _filter == 1, onSelected: (_) => setState(() => _filter = 1)),
              ChoiceChip(label: Text(t('requests_filter_mechanic')), selected: _filter == 2, onSelected: (_) => setState(() => _filter = 2)),
            ],
          ),
          ...
          if (_filter != 2)
            StreamBuilder<List<DeliveryMission>>(
              ...
              builder: (context, snapshot) {
                    ...
                    child: Center(
                      child: Column(
                        children: [
                          ...
                          Text(t('requests_loading'), style: const TextStyle(color: AppColors.textSecondary)),
                        ],
                      ),
                    ),
                  );
                ...
                if (missions.isEmpty && (_filter == 1 || mechanicJobs.isEmpty)) {
                  return Padding(
                    padding: const EdgeInsets.symmetric(vertical: 40),
                    child: Center(child: Text(t('requests_empty'), style: const TextStyle(color: AppColors.textSecondary))),
                  );
                }
                return Column(children: missions.map((m) => _DeliveryTile(mission: m, t: t)).toList());
              },
            ),
          if (_filter != 1)
...
class _DeliveryTile extends StatelessWidget {
  ...
  const _DeliveryTile({required this.mission, required this.t});
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(route, style: const TextStyle(color: AppColors.textSecondary, fontSize: 12), maxLines: 1, overflow: TextOverflow.ellipsis),
                  ],
                ),
              ),
              if (mission.customerTotal > 0)
                Text('${mission.customerTotal.toStringAsFixed(2)} \$', style: const TextStyle(fontWeight: FontWeight.w800)),
            ],
          ),
          const SizedBox(height: 12),
      ...
      child: Column(
        ...
        children: [
            ...
            children: [
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    Text(
                      request.selectedServices.isEmpty ? t('delivery_item_category') : (request.selectedServices as List<String>).map(t).join(', '),
                      style: const TextStyle(fontWeight: FontWeight.w700),
                      maxLines: 1,
          ...
          Container(
            ...
            child: Text(status.name, style: const TextStyle(fontSize: 11, color: AppColors.info, fontWeight: FontWeight.w700)),
          ),
        ],
      ),
    );
