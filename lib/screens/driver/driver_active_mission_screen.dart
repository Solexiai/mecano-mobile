...
class DriverActiveMissionScreen extends StatefulWidget {
  ...
  @override
  State<DriverActiveMissionScreen> createState() => _DriverActiveMissionScreenState();
}
...
class _DriverActiveMissionScreenState extends State<DriverActiveMissionScreen> {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        leading: IconButton(
          ...
          onPressed: () => context.canPop() ? context.pop() : context.go('/${context.read<LocaleProvider>().locale}/provider/dashboard'),
        ),
      ),
      body: SafeArea(
  ...
  Widget _buildBody(BuildContext context, String Function(String) t, FirebaseAuthProvider auth) {
    if (!auth.isSignedIn || auth.user == null) {
      return _MessageCard(
        ...
        onAction: () => context.go('/${context.read<LocaleProvider>().locale}/provider/dashboard'),
      );
    }
      ...
      builder: (context, snap) {
        ...
        return _MissionCard(
          ...
          onStartToPickup: () => _runAction(
            () => repo.updateTrackingStatus(missionId: mission.id, targetStatus: MissionStatus.driverToPickup),
          ),
          onArrivedAtPickup: () => _runAction(
            () => repo.updateTrackingStatus(missionId: mission.id, targetStatus: MissionStatus.arrivedAtPickup),
          ),
          onConfirmPickup: () => _runAction(() => repo.markPickupCompleted(mission.id)),
          onStartTransit: () => _runAction(
            () => repo.updateTrackingStatus(missionId: mission.id, targetStatus: MissionStatus.inTransit),
          ),
          onArrivedAtDropoff: () => _runAction(
            () => repo.updateTrackingStatus(missionId: mission.id, targetStatus: MissionStatus.arrivedAtDropoff),
          ),
          onCompleteDelivery: () => _runAction(() => repo.markDeliveryCompleted(mission.id)),
        );
      },
    );
  }
        ...
        if (errorKey != null) ...[
          ...
          Container(
            ...
            child: Row(children: [
              ...
              Expanded(child: Text(t(errorKey!), style: const TextStyle(fontSize: 12.5, color: AppColors.error))),
            ]),
          ),
        ],
  ...
  List<Widget> _buildActions(BuildContext context) {
    ...
    switch (mission.status) {
      case MissionStatus.assigned:
        return [button('driver_active_mission_start_to_pickup', onStartToPickup)];
      case MissionStatus.driverToPickup:
        return [button('driver_active_mission_arrived_at_pickup', onArrivedAtPickup)];
      ...
      case MissionStatus.inTransit:
        return [button('driver_active_mission_arrived_at_dropoff', onArrivedAtDropoff)];
      case MissionStatus.arrivedAtDropoff:
        return [button('driver_active_mission_mark_delivered', onCompleteDelivery)];
      default:
        return const [];
    }
  }
